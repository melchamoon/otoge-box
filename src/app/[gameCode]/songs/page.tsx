...
export default function SongsPage() {
  ...
  const columns = useMemo(
    () => [
      { id: "songNo", accessorKey: "songNo", header: "No." },
      {
        ...
        header: t("term.category"),
        sortingFn: (rowA: any, rowB: any, id: string) =>
          lookup.getCategoryIndex(rowA.getValue(id)) -
          lookup.getCategoryIndex(rowB.getValue(id)),
      },
      ...
      { id: "bpm", accessorKey: "bpm", header: t("term.bpm") },
      {
        ...
        sortingFn: (rowA: any, rowB: any, id: string) =>
          lookup.getVersionIndex(rowA.getValue(id)) -
          lookup.getVersionIndex(rowB.getValue(id)),
      },
    ],
    [lookup, t],
  );
  ...
  return (
    <div className="mx-auto max-w-screen-2xl px-4 py-6 sm:px-8">
      <h1 className="mb-5 text-3xl font-semibold">{t("page-title.songs")}</h1>
      <Input
        value={search}
                ...
                >
                  {row.getVisibleCells().map((cell: any) => (
                    <td key={cell.id} className="px-3 py-2">
                      {cell.column.id === "songNo" && song.songNo}
                      {cell.column.id === "category" &&
                        (song.category ?? "").replaceAll("|", "｜")}
                      {cell.column.id === "title" && (
                        <Link
                          href={`/${lookup.gameCode}/song/?id=${encodeURIComponent(song.songId ?? "")}`}
                          className="text-blue-600 underline"
                        >
                          {song.title}
                        </Link>
                      )}
                      {cell.column.id === "artist" && song.artist}
                      {cell.column.id === "sheets" && (
                        <div className="flex flex-wrap gap-1">
                          {song.sheets.map((sheet: any) => (
                            <Button
                              key={sheet.sheetExpr}
                              variant="ghost"
                              size="sm"
                              className="h-7 px-1 font-bold"
                              style={{
                                color: lookup.getDifficultyColor(
                                  sheet.difficulty,
                                ),
                              }}
                              onClick={() => open(sheet)}
                            >
                              {lookup.getTypeAbbr(sheet.type)} {sheet.level}
                            </Button>
                          ))}
                        </div>
                      )}
                      {cell.column.id === "bpm" && song.bpm}
                      {cell.column.id === "version" &&
                        lookup.getVersionAbbr(song.version)}
                    </td>
                  ))}
                </tr>
              );
