...
export default function GamePage() {
  ...
  useEffect(() => { const params = new URLSearchParams(window.location.search); if (params.size > 0) { const queryObject = Object.fromEntries(params.entries()); setFilters(loadFiltersFromQuery(queryObject)); sendEvent('FilterLinkLoaded'); router.replace(pathname); } }, [pathname, router, sendEvent]);
  const ritualReady = selectedSheets.length === 2 && selectedSheetSet.has(NULL_SHEET) && selectedSheetSet.has(VOID_SHEET);
  const clearMyList = async () => { if (!ritualReady) { setSelectedSheets([]); return; } setTheme(resolvedTheme === 'dark' ? 'light' : 'dark'); await new Promise((resolve) => window.setTimeout(resolve, 1500)); openSheet(HYBRID_SHEET); confetti({ particleCount: 100, spread: 60, origin: { y: 0.6 }, zIndex: 999 }); setSelectedSheets([HYBRID_SHEET]); sendEvent('SecretFound', { no: 3 }); };
