...
const localPath = (gameCode: string) => path.resolve(process.cwd(), 'public/local-data', gameCode, 'data.json');
...
export async function generateSitemaps() { return sites.filter((site) => !site.isHidden).map((site, id) => ({ id, gameCode: site.gameCode })); }
...
export default async function sitemap({ id }: { id: number }): Promise<MetadataRoute.Sitemap> {
  ...
  return [`/${site.gameCode}/`, `/${site.gameCode}/timeline/`, `/${site.gameCode}/gallery/`, `/${site.gameCode}/songs/`, `/${site.gameCode}/about/`, ...data.songs.map((song) => `/${site.gameCode}/song/?id=${encodeURIComponent(song.songId ?? '')}`)].map((url) => ({ url: new URL(url, base).toString() }));
}
