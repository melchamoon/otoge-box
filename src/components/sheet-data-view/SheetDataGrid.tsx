...
export function SheetDataGrid({ table }: { table: any }) {
  ...
  const setSortColumn = (id: string) =>
    table.setSorting([{ id, desc: id === activeColumn ? activeDesc : false }]);
  const toggleSortDirection = () =>
    table.setSorting([{ id: activeColumn, desc: !activeDesc }]);
  return (
    <div>
      <div className="mb-3 flex flex-wrap items-center justify-center gap-2">
        <label className="text-sm" htmlFor="grid-sort">
          {t("ui.sortBy")}
        </label>
        <Select
          id="grid-sort"
        ...
        >
          {activeDesc ? t("ui.sort-desc") : t("ui.sort-asc")}
        </Button>
      </div>
      <div
      ...
      >
        {table.getRowModel().rows.map((row: any) => (
          <SheetTile
            key={row.id}
            sheet={row.original as Sheet}
            filterMode={filterMode}
            onClick={() => open(row.original as Sheet)}
          />
        ))}
        {table.getRowModel().rows.length === 0 && (
          <p className="py-8 opacity-70">No data</p>
        )}
      </div>
    </div>
  );
