...
export function useSelectedSheets() {
  ...
  const selectedSheets = useMyListStore((state) => state.byGame[gameCode] ?? EMPTY_SHEETS);
  ...
  const toggleSheetSelection = (sheet: Sheet) => toggle(gameCode, getCanonicalSheet(sheet));
  return { selectedSheets, selectedSheetSet, toggleSheetSelection, setSelectedSheets: (sheets: Sheet[]) => setSheets(gameCode, sheets) };
}
