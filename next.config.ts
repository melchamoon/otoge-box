...
const nextConfig: NextConfig = {
  ...
  env: {
    ...
    NEXT_PUBLIC_SITE_TITLE: process.env.NEXT_PUBLIC_SITE_TITLE ?? process.env.SITE_TITLE ?? '音ゲーぼっくす',
    NEXT_PUBLIC_SITE_URL: process.env.NEXT_PUBLIC_SITE_URL ?? process.env.SITE_URL ?? 'http://localhost:3000',
    NEXT_PUBLIC_SITE_REPORT_URL: process.env.NEXT_PUBLIC_SITE_REPORT_URL ?? process.env.SITE_REPORT_URL ?? '',
