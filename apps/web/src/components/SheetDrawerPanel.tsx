...
export function SheetDrawerPanel() {
  ...
  const draw = () => {
    ...
    openCombo(pool, {
      isDrawPool: true,
      isDrawMode: true,
      headerTitle: t("sfc.SheetComboDialog.drawResults"),
    });
  };

  const toggleLights = async (index: number) => {
    const next = lights.map((light, lightIndex) =>
      Math.abs(lightIndex - index) <= 1 ? !light : light,
    );
    setLights(next);
  ...
  return (
    <div className="my-4 flex flex-wrap items-center justify-center gap-2 rounded border border-[var(--border)] bg-[var(--card)] p-3">
      <div className="flex gap-1">
        {(["light", "single", "combo"] as const).map((entry) => (
          <Button
            key={entry}
      ...
      {mode === "light" ? (
        ...
        >
          {lights.map((light, index) => (
            <Button
              ...
              onClick={() => void toggleLights(index)}
            >
              {light ? "💡" : "🕯"}
            </Button>
          ))}
        ...
        <Button onClick={draw} disabled={singleDrawer.isDrawing}>
          {singleDrawer.isDrawing
            ? "Drawing…"
            : mode === "combo"
              ? t("sfc.SheetDrawerPanel.drawRandomSheetCombo")
              : t("sfc.SheetDrawerPanel.drawRandomSheet")}
