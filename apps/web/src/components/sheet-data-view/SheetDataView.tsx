...
function Pagination({ table }: { table: any }) {
  return (
    <div className="my-3 flex items-center justify-center gap-2 text-sm">
      ...
      <span>
        {(table.state?.pagination?.pageIndex ?? 0) + 1} / {table.getPageCount()}
      </span>
      <button
        type="button"
        className="rounded border px-2 py-1 disabled:opacity-40"
...
export function SheetDataView({
  ...
  sheets: Sheet[];
  displayMode: "grid" | "table" | "chart";
}) {
  const columns = useSheetColumns();
  const [sorting, setSorting] = useState<any[]>([]);
  const [pagination, setPagination] = useState({ pageIndex: 0, pageSize: 48 });
  const data = useMemo(() => sheets, [sheets]);
  ...
  return (
    <div>
      ...
      {displayMode === "grid" ? (
        <SheetDataGrid table={table} />
      ) : (
        <SheetDataTable table={table} />
      )}
      <Pagination table={table} />
    </div>
  );
