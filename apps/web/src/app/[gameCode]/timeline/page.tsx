...
export default function TimelinePage() {
  ...
  const versionMap = useMemo(
    () => new Map(data.versions.map((version) => [version.version, version])),
    [data.versions],
  );
  const blocks = useMemo<TimelineBlock[]>(() => {
    const all: TimelineBlock[] = [
      ...data.versions.toReversed().map((version) => ({
        ...
        releaseDateValue: version.releaseDate
      ...
      ...[
        ...Map.groupBy(
          data.songs.toReversed().filter((song) => song.releaseDate != null),
      ...
      ...[
        ...Map.groupBy(
          data.songs.toReversed().filter((song) => song.releaseDate == null),
          (song) =>
            song.version != null ? versionMap.get(song.version) : null,
        ).entries(),
      ]
        .map(([version, songs]) => ({
          type: "songs" as const,
  ...
  return (
    <div className="mx-auto max-w-screen-xl px-4 py-6 sm:px-8">
      ...
      {data.regions.length > 0 && (
        <Select
          value={region ?? data.regions[0]?.region ?? ""}
      ...
      <Timeline
        ...
        onSongClick={(song) =>
          openCombo(song.sheets, { headerTitle: song.title })
        }
      />
    </div>
  );
