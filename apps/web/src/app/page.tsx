...
export default function HomePage() {
  ...
  return (
    <>
      {(logoActive || dark) && (
        <link rel="preload" href={RICK_SHEET.imageUrl} as="image" />
      )}
      <div className="mx-auto max-w-screen-2xl px-4 py-8 sm:px-8">
        <div className="grid gap-10 lg:grid-cols-[minmax(260px,0.8fr)_minmax(0,2fr)] lg:items-start">
          <section className="text-center lg:sticky lg:top-24">
            <button
              type="button"
              className="mx-auto block select-none rounded-full p-3"
              onPointerDown={startSecret}
              onPointerUp={stopSecret}
              onPointerLeave={stopSecret}
              onPointerCancel={stopSecret}
              aria-label="音ゲーぼっくす logo"
            >
              <Icon
                path={mdiMusicBoxMultiple}
                size={120}
                className={dark ? "dark-style" : ""}
                style={{
                  transform: logoActive ? "rotate(1845deg)" : undefined,
              ...
              {sites
                ...
                .map((site) => (
                  <Link
                    key={site.gameCode}
                    href={`/${site.gameCode}/`}
                    className="group flex items-center gap-4 rounded-lg border border-[var(--border)] bg-[var(--card)] p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg"
                  >
                    <Icon
                      path={mdiMusicBoxMultiple}
                      size={38}
      ...
      <Dialog open={secretOpen} onOpenChange={setSecretOpen}>
        <DialogContent>
          ...
          <img
            src={RICK_SHEET.imageUrl}
            alt={RICK_SHEET.title}
            className="mx-auto max-h-[55vh] max-w-full object-contain"
          />
          <p className="mt-4 whitespace-pre-wrap text-sm opacity-80">
            {RICK_SHEET.comment}
          </p>
        </DialogContent>
      </Dialog>
