...
export async function getCookies() {
  ...
  throw new Error(
    "Please set your GITADORA_JP_KONAMI_SESSION_TOKEN in the .env file",
  );
}

async function fetchSongInfos(
  gameType: "gf" | "dm",
  categoryNo: number,
  cookies: Record<string, string>,
) {
  const response = await axios.get(
    `${DATA_URL}/game/gfdm/${VERSION_ID}/p/playdata/music.html`,
    {
      headers: {
  ...
  if (
    $(':contains("このページのご利用にはe-amusementへのログインが必要です。")')
      .length !== 0
  ) {
    throw new Error("Login expired. Please run again to continue.");
  }

  const songInfos = $("table.music_table_tb .music_cell")
    .toArray()
    .map((musicCell) => {
       ...
       */
      const transientDetailUrl = new URL(
        $(musicCell).find(".title_box a.text_link").attr("href")!,
        DATA_URL,
      ).toString();
  ...
  if (songInfos.length === 0) {
    logger.warn(
      "No songs found on this page. The login may have already expired.",
    );
  }
...
async function fetchSong(
  ...
  if (title !== songInfo.title) {
    throw new Error(
      `Title mismatch detected. The song list may already changed. (old: ${songInfo.title}, new: ${title})`,
    );
  }
   ...
   */
  const transientImageUrl = new URL(
    $("img.music_jacket").attr("src")!,
    IMAGE_BASE_URL,
  ).toString();
...
async function fetchSongs(
  ...
  assertNonEmpty(`gitadora ${gameType} songs`, songInfos);
  ...
  for (const [index, songInfo] of songInfos.entries()) {
    if (!forceRefetch) {
      ...
      const existSheets = await Sheet.findAll({
        where: {
          songId: songInfo.songId,
          type:
            songInfo.gameType === "gf"
              ? ["guitar", "bass"]
              : songInfo.gameType === "dm"
                ? ["drum"]
                : undefined,
        },
      });
    ...
    logger.info(
      `(${1 + index} / ${songInfos.length}) Fetching data & image: ${songInfo.title} ...`,
    );
    ...
    if (song.imageName && !fs.existsSync(`${coverImgDir}/${song.imageName}`)) {
      await download(song.transientImageUrl, coverImgDir, {
        filename: song.imageName,
        headers: { Cookie: `M573SSID=${cookies.M573SSID};` },
      });
      await sleep(100);
    }
    if (
