...
function getWeType(rawSong: Record<string, any>) {
  ...
  const match = (rawSong.meta.title as string).match(
    /【(?<weType>[^【】]+)】$/,
  );
...
function extractSheets(rawSong: Record<string, any>) {
  return [
    ...
    .filter((e) => !!e.data && !e.data.is_const_unknown && e.data.const !== 0)
    .map((rawSheet) => ({
...
async function fetchSongs() {
  if (!process.env.CHUNITHM_CHUNIREC_ACCESS_TOKEN) {
    throw new Error(
      "Please set your CHUNITHM_CHUNIREC_ACCESS_TOKEN in the .env file",
    );
  }
