...
import { replaceTable } from "@/_core/database";
...
async function getIntlCookies() {
  if (
    !process.env.CHUNITHM_INTL_SEGA_ID ||
    !process.env.CHUNITHM_INTL_SEGA_PASSWORD
  ) {
    throw new Error(
      "Please set your CHUNITHM_INTL_SEGA_ID and CHUNITHM_INTL_SEGA_PASSWORD in the .env file",
    );
  }
...
async function getIntlSheets(
  ...
  const response = await axios.get(`${DATA_URL}/musicGenre/${difficulty}`, {
    headers: {
      Cookie: `userId=${cookies.userId}; _t=${cookies._t};`,
    },
  });
...
async function getIntlWorldsEndSheets(cookies: Record<string, string>) {
  const response = await axios.get(`${DATA_URL}/worldsEndList/`, {
    headers: {
      Cookie: `userId=${cookies.userId}; _t=${cookies._t};`,
    },
  });
  ...
  return sheetBlocks.map((e) => {
    ...
    const weTypeId = $(e)
      .find(".musiclist_worldsend_icon img")
      .attr("src")!
      .match(
        /^https:\/\/chunithm-net-eng.com\/mobile\/images\/icon_we_(\d+).png/,
      )![1];
...
export default async function run() {
  ...
  ensureNoDuplicateEntry(
    intlSheets.map((sheet) =>
      [sheet.songId, sheet.type, sheet.difficulty].join("|"),
    ),
  );

  logger.info("Truncating and Inserting intlSheets ...");
  await replaceTable(IntlSheet, intlSheets);
