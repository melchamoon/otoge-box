...
function extractSong(rawSong: Record<string, any>) {
  ...
  return {
    ...
    releaseDate: null,
    sortOrder:
      Number(rawSong.id) < 8000
        ? Number(rawSong.id)
        : Number(rawSong.id) - 10000,
...
function extractSheets(rawSong: Record<string, any>) {
  return [
    ...
    { type: "std", difficulty: "ultima", level: rawSong.lev_ult },
    {
      ...
      level:
        rawSong.we_star !== ""
          ? "☆".repeat((Number(rawSong.we_star) + 1) / 2)
          : null,
    },
  ]
    .filter((e) => !!e.level)
...
export default async function run() {
  ...
  assertNonEmpty("chunithm songs", songs);
  assertNonEmpty("chunithm sheets", sheets);
