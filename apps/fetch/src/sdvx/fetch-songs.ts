...
function getSongId(rawSong: Record<string, any>) {
  ...
  if (title === "カジノファイヤーことみちゃん") {
    if (artist === "ARM feat. 山本椛 + Brasscapsule")
      return "カジノファイヤーことみちゃん";
    if (artist === "covered by 一条莉々華(ReGLOSS)")
      return "カジノファイヤーことみちゃん (2)";
  }
  if (title === "朱と碧のランページ") {
    if (artist === "NU-KO") return "朱と碧のランページ";
    if (artist === "covered by 儒烏風亭らでん(ReGLOSS)")
      return "朱と碧のランページ (2)";
  }
  return title;
}
...
async function* getSongs() {
  ...
  async function* startFetchPage(
    ...
    const songs = $("#music-result .music")
      .toArray()
      .map((div) => {
        const category = $(div)
          .find(".genre")
        ...
        const title = $(div)
          .find(".info p:nth-of-type(1)")
          .text()
          .trim()
          .replaceAll("\u00A0", " ");
        const artist = $(div)
          .find(".info p:nth-of-type(2)")
          .text()
          .trim()
          .replaceAll("\u00A0", " ");
        ...
        const sheets = $(div)
          ...
          .map((e) => {
            ...
            if (difficulty === undefined) {
              throw new Error(
                `'${difficultyAbbr}' cannot be mapped to a valid difficulty.`,
              );
            }
...
export default async function run() {
  ...
  assertNonEmpty("sdvx songs", songs);
  assertNonEmpty("sdvx sheets", sheets);
