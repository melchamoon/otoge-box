...
async function* getPages(listId: string) {
  ...
  async function* startFetchPage(
    ...
    const songs = $("#music_list .list_data")
      .toArray()
      .map((div) => {
        ...
        const title = $(div)
          .find(".list_data > ul > li:nth-of-type(1)")
          .text()
          .trim();
        const artist = $(div)
          .find(".list_data > ul > li:nth-of-type(2)")
          .text()
          .trim();
        ...
        const levels = $(div)
          .find(".list_data > ul > li:nth-of-type(3) > ul > li:nth-of-type(2n)")
          .toArray()
          .map((e) => $(e).text().trim());
...
export default async function run() {
  ...
  assertNonEmpty("jubeat songs", songs);
  assertNonEmpty("jubeat sheets", sheets);
