...
export async function getCookies() {
  if (!process.env.DIVA_JP_SEGA_ID || !process.env.DIVA_JP_SEGA_PASSWORD) {
    throw new Error(
      "Please set your DIVA_JP_SEGA_ID and DIVA_JP_SEGA_PASSWORD in the .env file",
    );
  }
...
async function* fetchPages(cookies: Record<string, string>) {
  async function* startFetchPage(
    ...
    const response = await axios.get(
      `${DATA_URL}/divanet/pv/sort/0/false/${pageNo}`,
      {
        headers: {
    ...
    if (
      $(':contains("データ保護のためDIVA.NETサーバへの接続を終了します。")')
        .length !== 0
    ) {
      throw new Error("Login expired. Please run again to continue.");
    }

    const songInfos = $('a[href^="/divanet/pv/info/"]')
      .toArray()
      .map((e) => {
...
async function fetchSong(
  ...
  const artist = $(
    "center:nth-of-type(3) > table:last-of-type > tbody > tr:nth-child(1) > td:nth-child(2)",
  )
    .text()
    .trim();
  ...
  const sheets = new Map(
    $(
      "center:nth-of-type(3) > table:not(:last-of-type) > tbody > tr:nth-child(1) > td:nth-child(1)",
    )
      .toArray()
      .map((e) => {
...
export default async function run() {
  ...
  assertNonEmpty("diva songs", songInfos);
  ...
  for (const [index, songInfo] of songsToFetch.entries()) {
    try {
      logger.info(
        `(${1 + index} / ${songsToFetch.length}) Updating song & sheets for: ${songInfo.title} ...`,
      );
  ...
  if (errors.length > 0) {
    throw new AggregateError(errors, "One or more Project DIVA pages failed.");
  }
