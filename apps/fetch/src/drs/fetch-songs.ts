...
function getSongId(rawSong: Record<string, any>) {
  ...
  if (title === "Touch Me") {
    ...
    if (artist === 'BEMANI Sound Team "GekiReco ＆ Sota F."')
      return "Touch Me (2)";
  }
  return title;
}
  ...
  return [
    {
      type: "std",
      difficulty: "easy",
      level: rawSong.difficulty.fumen_1b?.difnum,
    },
    {
      type: "std",
      difficulty: "normal",
      level: rawSong.difficulty.fumen_1a?.difnum,
    },
    {
      type: "dual",
      difficulty: "easy",
      level: rawSong.difficulty.fumen_2b?.difnum,
    },
    {
      type: "dual",
      difficulty: "normal",
      level: rawSong.difficulty.fumen_2a?.difnum,
    },
  ]
    .filter((e) => !!e.level)
    .map((rawSheet) => ({
...
export default async function run() {
  ...
  assertNonEmpty("drs songs", songs);
  assertNonEmpty("drs sheets", sheets);
