...
import { assertNonEmpty, replaceTable } from "@/_core/database";
...
function getSheetType(rawSong: Record<string, any>) {
  ...
  if (title === "Popperz Chronicle") {
    if (
      id === "yXzsRjrwR++OJz1rSsRV8w==" ||
      id === "0VU9Chj0Ll0A3XT8g9+AZ5Z9T42mSjB6eFKSHZ+nGTYj9f52y177fYXh2DN7J6rn"
    )
      return "std";
    if (
      id === "cujBwQyPLOzCSiDEqkwP+A==" ||
      id === "0VU9Chj0Ll0A3XT8g9+AZxVdrZ7tOmApq3UHeQLfIyEj9f52y177fYXh2DN7J6rn"
    )
      return "upper";
  ...
  if (title === "virkatoの主題によるperson09風超絶技巧変奏曲") {
    if (
      id === "ERthT9SqJgNYCMFGla/7Ng==" ||
      id === "0VU9Chj0Ll0A3XT8g9+AZxfldwl3AUHq9pDCOMMfVoEj9f52y177fYXh2DN7J6rn"
    )
      return "std";
    if (
      id === "26l7SvtoJR4pmZz8C9yTWQ==" ||
      id === "0VU9Chj0Ll0A3XT8g9+AZ9zUAz1z5/CE3vxdBLggTtoj9f52y177fYXh2DN7J6rn"
    )
      return "upper";
  ...
  if (manualUpperMappingWithVersion.has(title)) {
    ...
    throw new Error(
      `Cannot use manual mapping with version: ${JSON.stringify(rawSong)}`,
    );
  }
...
export async function getCookies() {
  ...
  throw new Error(
    "Please set your POPN_JP_KONAMI_SESSION_TOKEN in the .env file",
  );
}

async function* fetchSongs(
  versionOrCategoryId: number,
  ...
  async function* startFetchPage(
    ...
    const response = await axios.get(
      `${DATA_URL}/game/popn/${VERSION_ID}/playdata/mu_top.html`,
      {
        headers: {
    ...
    const songs = $(".mu_list_table > li")
      .toArray()
      .slice(1)
      .map((li) => {
        const detailUrl = new URL(
          $(li).find(".col_music > a").attr("href")!,
          DATA_URL,
        ).toString();
...
async function* fetchSheets(
  ...
  async function* startFetchPage(
    ...
    const response = await axios.get(
      `${DATA_URL}/game/popn/${VERSION_ID}/playdata/mu_lv.html`,
      {
        headers: {
          Cookie: `M573SSID=${cookies.M573SSID};`,
        },
    ...
    const sheets = $(".mu_list_table > li")
      .toArray()
      .slice(1)
      .map((li) => {
        const id = new URL(
          $(li).find(".col_music_lv > a").attr("href")!,
          DATA_URL,
        ).searchParams.get("no");
        const title = $(li)
          .find(".col_music_lv > a")
...
async function* fetchSongArtists(
  ...
  const songsToFetch = songs.filter(
    (song) =>
      !existedSongArtists.some(
        (songArtist) => songArtist.songId === song.songId,
      ),
  );
  ...
  for (const [index, song] of songsToFetch.entries()) {
    try {
      logger.info(
        `(${1 + index} / ${songsToFetch.length}) Updating song artist for: ${song.title} ...`,
      );
...
export default async function run() {
  ...
  assertNonEmpty("pop'n songs", songs);
  assertNonEmpty("pop'n sheets", sheets);
  ...
  for await (const songArtist of fetchSongArtists(songs, cookies, errors)) {
    await SongArtist.upsert(songArtist);
  }

  if (errors.length > 0) {
    throw new AggregateError(errors, "One or more pop'n artist pages failed.");
  }
