...
function getSongId(rawSong: Record<string, any>) {
  ...
  if (title === "Singularity") {
    ...
    if (artist === "SEGA SOUND STAFF「セガNET麻雀 MJ」")
      return "Singularity (3)";
  }
  if (title === "Hand in Hand") {
    if (artist === "ユーフィリア(CV：高橋 李依)「アンジュ・ヴィエルジュ」")
      return "Hand in Hand";
...
function extractSong(rawSong: Record<string, any>) {
  ...
  return {
    ...
    releaseDate: rawSong.date
      ? `${rawSong.date.substring(0, 4)}-${rawSong.date.substring(
          4,
          6,
...
export default async function run() {
  ...
  assertNonEmpty("ongeki songs", songs);
  assertNonEmpty("ongeki sheets", sheets);
