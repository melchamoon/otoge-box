  ...
  return trs
    .map((tr) => {
      ...
      return {
        ...
        noteDesigner:
          noteDesigner ||
          (["basic", "advanced"].includes(difficulty) ? "-" : null),
      };
    })
...
async function fetchExtra(song: Record<string, any>, pageUrl: string) {
  ...
  const sheetExtras = [$('h2:contains("詳細") + div > table').get(0)]
...
export default async function run() {
  ...
  if (errors.length > 0) {
    throw new AggregateError(errors, "One or more ONGEKI extra pages failed.");
  }
