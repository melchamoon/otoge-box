...
async function extractRecords({
  ...
  return dataIndexes.flatMap((dataIndex) => {
    const [titleIndex, typeIndex, difficultyIndex, internalLevelIndex] =
      dataOffsets.map((offset) => dataIndex + offset);

    const rawSheets = [...Array(sheet.rowCount).keys()]
      .filter((rowIndex) => {
        const internalLevelValue = sheet.getCell(
          rowIndex,
          internalLevelIndex,
        ).value;
      ...
      .map((rowIndex) => ({
        ...
        difficulty: difficultyMapping.get(
          String(sheet.getCell(rowIndex, difficultyIndex).value),
        ),
        internalLevel: Number(
          sheet.getCell(rowIndex, internalLevelIndex).value,
        ).toFixed(1),
      }));
...
export async function fetchSheetsV6() {
  const spreadsheet = new GoogleSpreadsheet(
    "1byBSBQE547KL2KzPkUjY45svcIrJeHh57h-DLJycQbs",
...
export async function fetchSheetsV7() {
  const spreadsheet = new GoogleSpreadsheet(
    "1xbDMo-36bGL_d435Oy8TTVq4ADFmxl9sYFqhTXiJYRg",
    { apiKey: process.env.GOOGLE_API_KEY! },
  );
  await spreadsheet.loadInfo();
...
export async function fetchSheetsV8() {
  const spreadsheet = new GoogleSpreadsheet(
    "1xqXfzfDfxiEE9mREwgX_ITIY8AowRM7w-TH2t1I_RJE",
    { apiKey: process.env.GOOGLE_API_KEY! },
  );
  await spreadsheet.loadInfo();
...
export async function fetchSheetsV9() {
  const spreadsheet = new GoogleSpreadsheet(
    "1vSqx2ghJKjWwCLrDEyZTUMSy5wkq_gY4i0GrJgSreQc",
    { apiKey: process.env.GOOGLE_API_KEY! },
  );
  await spreadsheet.loadInfo();
...
export async function fetchSheetsV13() {
  const spreadsheet = new GoogleSpreadsheet(
    "17vd35oIHxjXPUU-6QJwYoTLPs2nneHN4hokMNLoQQLY",
    { apiKey: process.env.GOOGLE_API_KEY! },
  );
  await spreadsheet.loadInfo();
...
export async function fetchSheetsV14() {
  const spreadsheet = new GoogleSpreadsheet(
    "1JXFhqpow60lXYzETOXaqIRVIaIpWWxCsGCcE0piLLDw",
    { apiKey: process.env.GOOGLE_API_KEY! },
  );
  await spreadsheet.loadInfo();
