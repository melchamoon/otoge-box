...
function getLevelValueOf(sheet: Record<string, any>) {
  ...
  if (sheet.level.endsWith("?"))
    return Number(sheet.level.replace("?", "").replace("+", ".6"));
  ...
  for (const sheetRecord of sheetRecords as any[]) {
    ...
    for (const region of Object.keys(sheetRecord.regionOverrides)) {
      ...
      for (const key of Object.keys(regionOverride)) {
        if (
          regionOverride[key] == null ||
          regionOverride[key] === sheetRecord[key]
        ) {
      ...
      if ("level" in regionOverride) {
        regionOverride.levelValue = getLevelValueOf({
          level: regionOverride.level,
        });
      }
    }

    sheetRecord.regions.usa =
      sheetRecord.regions.intl && !usaHiddenSongSet.has(sheetRecord.songId);
  }
