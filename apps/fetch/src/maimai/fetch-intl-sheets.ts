...
export async function getIntlCookies() {
  if (
    !process.env.MAIMAI_INTL_SEGA_ID ||
    !process.env.MAIMAI_INTL_SEGA_PASSWORD
  ) {
    throw new Error(
      "Please set your MAIMAI_INTL_SEGA_ID and MAIMAI_INTL_SEGA_PASSWORD in the .env file",
    );
  }
...
async function getIntlSheets(
  ...
  return sheetBlocks.map((e) => {
    ...
    const type = (() => {
      const typeButton = $(e).siblings(
        `.music_kind_icon, .music_${difficulty}_btn_on`,
      );
...
async function getIntlUtageSheets(cookies: Record<string, string>) {
  const response = await axios.get(DATA_URL, {
    headers: {
      Cookie: `userId=${cookies.userId};`,
    },
...
export default async function run() {
  ...
  assertNonEmpty("maimai intl sheets", intlSheets);
