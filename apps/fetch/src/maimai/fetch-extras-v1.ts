...
const DATA_URL =
  "https://web.archive.org/web/20200718222454/http://maimai.wiki.fc2.com/wiki";
...
async function fetchExtra(song: Record<string, any>) {
  ...
  const bpm =
    Number.parseFloat($('td:contains("BPM")').next().text().trim()) ?? null;
...
export default async function run() {
  ...
  for (const [index, song] of songsToFetch.entries()) {
    try {
      logger.info(
        `(${1 + index} / ${songsToFetch.length}) Updating extra for: ${song.title} ...`,
      );
