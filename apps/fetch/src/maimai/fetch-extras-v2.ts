...
function extractSheetExtras($: cheerio.CheerioAPI, table: Element) {
  const noInternalLevel =
    $(table).find("tr").eq(0).find("th").eq(1).text().trim() === "総数";

  const type = (() => {
    const fourthNoteType = $(table)
      .find("tr")
      .eq(1)
      .find("th")
      .eq(3)
  ...
  return trs
    .map((tr) => {
      ...
      const difficultyColor = tds[0].attribs.style.match(
        "background-color:(#[0-9a-f]+)",
      )![1];

      const [
        ,
        ,
        /* level */ /* internalLevel */ totalCount,
        tapCount,
        holdCount,
        slideCount,
        touchCount,
      ...
      if (difficulty === undefined) {
        throw new Error(
          `'${difficultyColor}' is not a valid difficulty color.`,
        );
      }
      ...
      const noteDesigner = (() => {
        ...
        const extractNoteDesigner = (abbr: string) => {
          const anchorIndex = nodes.findIndex(
            (node) => $(node).text().trim() === abbr,
          );
          if (anchorIndex === -1) return null;

          let result = $(nodes[anchorIndex + 1])
            .text()
            .trim()
            .replace(/^[…]|[、】]$/g, "")
            .trim();
        ...
        if (difficulty === "master")
          return extractNoteDesigner("MAS") ?? extractNoteDesigner("MST");
...
async function fetchExtra(song: Record<string, any>, pageUrl: string) {
  ...
  const bpm =
    Number.parseFloat($('th:contains("BPM")').next().text().trim()) || null;
  ...
  const sheetExtras = [
    ...
    .flatMap((table) =>
      extractSheetExtras($, table).map((sheetExtra) => ({
        songId: song.songId,
        ...sheetExtra,
      })),
    );
...
export default async function run() {
  ...
  const songsToFetch: Record<string, any>[] = await sequelize.query(
    /* sql */ `
  ...
  for (const [index, song] of songsToFetch.entries()) {
    try {
      logger.info(
        `(${1 + index} / ${songsToFetch.length}) Fetching & Updating extra: ${song.title} ...`,
      );
      const pageUrl = songPageUrlMap.get(getSongWikiTitle(song));

      if (pageUrl === undefined)
        throw new Error(`Song page url not found on ${SONG_LIST_URL}`);

      const { songExtra, sheetExtras } = await fetchExtra(song, pageUrl);

      await SongExtra.upsert(songExtra);
      await Promise.all(
        sheetExtras.map((sheetExtra) => SheetExtra.upsert(sheetExtra)),
      );
    } catch (e: any) {
  ...
  if (errors.length > 0) {
    throw new AggregateError(errors, "One or more maimai extra pages failed.");
  }
