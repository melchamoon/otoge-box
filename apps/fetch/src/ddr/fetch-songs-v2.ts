...
async function getCookies() {
  ...
  throw new Error(
    "Please set your DDR_JP_KONAMI_SESSION_TOKEN in the .env file",
  );
}
...
async function* getVersions() {
  ...
  for (const { versionId, version } of versions) {
    ...
    async function* startFetchPage(
      ...
      if (
        $(':contains("ご利用にはe-amusement へのログインが必要です。")')
          .length !== 0
      ) {
      ...
      const songs = $("#data_tbl tr.data")
        ...
        .map((tr) => {
          const title = $(tr)
            .find("td:nth-of-type(1) .music_info")
            .text()
            .trim();
...
export default async function run() {
  ...
  // inject category info
  {
    ...
    for await (const pageOfSongs of getCategories()) {
      for (const song of pageOfSongs) {
        if (!songCategoryMapping.has(song.songId))
          songCategoryMapping.set(song.songId, []);
  ...
  // sort songs by version
  {
    ...
    songs.sort(
      (a, b) =>
        0 || versionOrder.get(a.version)! - versionOrder.get(b.version)!,
    );
  }

  const sheets = songs.flatMap((song) => extractSheets(song));
  assertNonEmpty("ddr songs", songs);
  assertNonEmpty("ddr sheets", sheets);
