...
function getSongId(rawSong: Record<string, any>) {
  ...
  if (title === "エンジェル ドリーム") {
    if (artist === "「アイドルマスター シンデレラガールズ」より")
      return "エンジェル ドリーム";
    if (artist === null) return "エンジェル ドリーム (2)";
    if (artist === "フレン・E・ルスタリオ / FOCUS ON (にじさんじ) × 太鼓の達人")
      return "エンジェル ドリーム (3)";
  }
  if (title === "ファミレスウォーズ") {
    if (artist === null) return "ファミレスウォーズ";
    if (artist === "本間ひまわり / FOCUS ON (にじさんじ) × 太鼓の達人")
      return "ファミレスウォーズ (2)";
  }
  if (title === "Fly away") {
    if (artist === null) return "Fly away";
    if (artist === "伏見ガク / FOCUS ON (にじさんじ) × 太鼓の達人")
      return "Fly away (2)";
  }
  if (title === "濃紅") {
    if (artist === "黒沢ダイスケ × 小寺可南子") return "濃紅";
    if (artist === "レヴィ・エリファ / FOCUS ON (にじさんじ) × 太鼓の達人")
      return "濃紅 (2)";
  }
  if (title === "CYBERgenicALICE") {
    if (artist === null) return "CYBERgenicALICE";
    if (artist === "先斗寧 / FOCUS ON (にじさんじ) × 太鼓の達人")
      return "CYBERgenicALICE (2)";
  }
  if (title === "マリオネットピュア") {
    if (artist === null) return "マリオネットピュア";
    if (artist === "リゼ・ヘルエスタ / FOCUS ON (にじさんじ) × 太鼓の達人")
      return "マリオネットピュア (2)";
  }
  if (title === "Phoenix") {
    if (artist === null) return "Phoenix";
    if (artist === "レオス・ヴィンセント / FOCUS ON (にじさんじ) × 太鼓の達人")
      return "Phoenix (2)";
  }
  if (title === "ヘイラ") {
    if (artist === "necchi") return "ヘイラ";
    if (artist === "ましろ爻 / FOCUS ON (にじさんじ) × 太鼓の達人")
      return "ヘイラ (2)";
  }
  if (title === "いっそこのままで") {
    if (artist === "ミフメイ(BNSI) feat. 相沢") return "いっそこのままで";
    if (artist === "山神カルタ / FOCUS ON (にじさんじ) × 太鼓の達人")
      return "いっそこのままで (2)";
  }
  if (title === "ボクハシンセ") {
    if (artist === null) return "ボクハシンセ";
    if (artist === "花畑チャイカ / FOCUS ON (にじさんじ) × 太鼓の達人")
      return "ボクハシンセ (2)";
  }
  if (title === "Just Awake") {
    if (artist === "Fear, and Loathing in Las Vegas 「HUNTER×HUNTER」より")
      return "Just Awake";
  ...
  const songs = section
    ...
    .map((tr) => {
      ...
      const rawSong = {
        ...
        isLocked:
          $th.find(".icoMedal, .icoAi, .icoCampaign, .icoCode, .icoSecrect")
            .length !== 0,
  ...
  for (const song of songs) {
    if (mergedSongs.has(song.songId)) {
      ...
      if (song.artist !== mergedSong.artist) {
        throw new Error(
          `Same songId with different artist detected: ${song.songId}`,
        );
      }
...
export default async function run() {
  ...
  assertNonEmpty("taiko songs", songs);
  assertNonEmpty("taiko sheets", sheets);
