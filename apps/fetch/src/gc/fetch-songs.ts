...
function extractCategories($: cheerio.CheerioAPI) {
  const categories = $(".music-nav nav ul li")
    .toArray()
    .map((li) => ({
  ...
  const songs = $(`${selector} ul li > a`)
    ...
    .map((a) => {
      ...
      if (
        ...
        const [realTitle, realArtist] = infoTexts[0]
          .split("／", 2)
          .map((s) => s.trim());
      ...
      if (infoTexts[0] === "Captain NEO -Confusion Mix-")
        infoTexts[4] = "2014.1.10";
      ...
      const releaseDate = release
        ? `${release[1].padStart(4, "0")}-${release[2].padStart(
            2,
            "0",
...
async function fetchSongExtra(song: Record<string, any>) {
  ...
  const bpm =
    Number.parseFloat($(".param-block .details ul .bpm").text().trim()) || null;

  const levels = $(".param-block .difficulty ul li")
    .toArray()
    .map(
      (ul) =>
        $(ul)
          .find("img")
          .attr("src")!
          .match(/^\/img\/music\/img_dif_(\d+).png$/)![1],
    );
...
export default async function run() {
  ...
  const songsToFetch = songs.filter(
    (song) =>
      ...
      (song.hasEx &&
        !existedExSheets.some((sheet) => sheet.songId === song.songId)) ||
  ...
  for (const [index, song] of songsToFetch.entries()) {
    try {
      logger.info(
        `(${1 + index} / ${songsToFetch.length}) Updating extra & sheets for: ${song.title} ...`,
      );
  ...
  if (errors.length > 0) {
    throw new AggregateError(
      errors,
      "One or more GROOVE COASTER pages failed.",
    );
  }
